...
use razel_exec_api::{ExecError, InputArtifact, SpawnRequest, SpawnResult, SpawnStrategy};
...
fn owner_ct(pkg: &str, name: &str) -> razel_analysis::ConfiguredTargetKey {
...
fn in_art(path: &str, content: &[u8]) -> InputArtifact {
    InputArtifact { path: path.into(), content: content.to_vec() }
}
fn spawn_req(mnemonic: &str, argv: &[&str], inputs: Vec<InputArtifact>, outputs: &[&str]) -> SpawnRequest {
    SpawnRequest::new(
        mnemonic,
...
def _impl(ctx):\n\
\x20   declare_action(mnemonic = \"Compile\", argv = [\"cc\", \"-o\", \"out\"], outputs = [\"app/out.txt\"], inputs = [\"app/in.txt\"])\n\
...
def _impl(ctx):\n\
\x20   declare_action(mnemonic = \"Gen\", argv = [\"gen\"], outputs = [\"app/mid.txt\"], inputs = [\"app/in.txt\"])\n\
\x20   declare_action(mnemonic = \"Pack\", argv = [\"pack\"], outputs = [\"app/out.txt\"], inputs = [\"app/mid.txt\"])\n\
\x20   return [NumberInfo(x = 1)]\n\
my_rule = rule(implementation = _impl, attrs = {})\n";
...
def _impl(ctx):\n\
\x20   declare_action(mnemonic = \"Compile\", argv = [\"cc\", \"-O2\", \"-o\", \"out\"], outputs = [\"app/out.txt\"], inputs = [\"app/in.txt\"])\n\
\x20   return [NumberInfo(x = 1)]\n\
my_rule = rule(implementation = _impl, attrs = {})\n";
...
def _impl(ctx):\n\
\x20   declare_action(mnemonic = \"A\", argv = [\"a\"], outputs = [\"app/dup.txt\"])\n\
\x20   declare_action(mnemonic = \"B\", argv = [\"b\"], outputs = [\"app/dup.txt\"])\n\
\x20   return [NumberInfo(x = 1)]\n\
my_rule = rule(implementation = _impl, attrs = {})\n";
...
def _impl(ctx):\n\
\x20   declare_action(mnemonic = \"Abs\", argv = [\"a\"], outputs = [\"app/o.txt\"], inputs = [\"/etc/passwd\"])\n\
\x20   return [NumberInfo(x = 1)]\n\
my_rule = rule(implementation = _impl, attrs = {})\n";
